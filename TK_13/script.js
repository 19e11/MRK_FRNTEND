const users = [
  ...
  users.forEach((nigga) => {
    jodd += ` <div class="card">
        <img class="img1" src="${nigga.coverPhoto}" alt=""> 
        <img class="img2" src="${nigga.profilePhoto}" alt="">
        <h1>${nigga.username}</h1>
       <h2>${nigga.fullName}</h2>
       <p>${nigga.description}</p>

       <br><i class="ri-git-commit-fill"></i>
       <div class="data">
         <h1>${nigga.mediaUploadedCount} <br> Media</h1>
         <h1>${nigga.followersCount} <br> Followers</h1>
         <h1>${nigga.followingCount} <br> Following</h1>
        </div>
     </div> `
  })    
